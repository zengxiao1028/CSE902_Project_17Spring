...
def evaluate_sd14():
    ...
    x_train, y_train, x_test, y_test = dg.get_batch(1,num_test_epochs=1)
    ...
    try:
        ...
        ckpt = tf.train.get_checkpoint_state('sd14_out/restore_models/')
        ...
        while not coord.should_stop():
            ...
            acc, feas, y_pred = sess.run([net.acc, net.fea, net.predction],
                                       feed_dict={net.x_ph:x_test_batch, net.y_ph:y_test_batch, net.is_training:False})
            ...
            if  y_test_batch!=y_pred:
                fail_cnt += 1
                save_case(y_test_batch,y_pred,x_test_batch,fail_cnt,'sd14_out/fails/')
    ...
    y_labels = np.vstack(y_labels).reshape((-1, 1))

    print(np.mean(mean_acc))

    #joblib.dump((x_features,y_labels),'sd14_train_features.pkl')

    joblib.dump((x_features, y_labels), 'sd14_test_features.pkl')

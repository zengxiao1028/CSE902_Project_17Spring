...
class GlimpseNet(object):
  ...
  def get_glimpse(self, loc):
    ...
    #resolution_1
    glimpse_imgs_1 = tf.image.extract_glimpse(imgs,
                                            [self.win_size, self.win_size], loc)

    glimpse_imgs_1 = tf.reshape(glimpse_imgs_1, [
        tf.shape(loc)[0], self.win_size * self.win_size * self.num_channels
    ])


    # resolution_2
    glimpse_imgs_2 = tf.image.extract_glimpse(imgs,
                                            [self.win_size*2, self.win_size*2], loc)

    glimpse_imgs_2 = tf.image.resize_images(glimpse_imgs_2,[self.win_size, self.win_size])

    glimpse_imgs_2 = tf.reshape(glimpse_imgs_2, [
        tf.shape(loc)[0], self.win_size * self.win_size * self.num_channels
    ])

    glimpse_imgs = tf.concat([glimpse_imgs_1,glimpse_imgs_2],1)
